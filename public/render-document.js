...
function renderQuestionCard(item, mode, includeAnswerKey = false) {
  const metaLine = `
    <div class="meta">
  ...
  const answerBlock =
    mode === EXPORT_MODES.teacher && !includeAnswerKey
      ? `
...
export function renderDocumentHtml({ batch, mode, includeAnswerKey = false, headerText }) {
  return `
    ...
    <html lang="en">
      ...
      <body>
        <div id="source-cards">
          ${batch.map((item) => renderQuestionCard(item, mode, includeAnswerKey)).join('')}
        </div>
        ${
          includeAnswerKey
            ? `
